...
if program == "blastn":
    ...
    e_value = 0.001
    blast_cline = NcbiblastnCommandline(query=infas, db=blastdb, evalue=e_value, outfmt=5, out=outfile, num_threads=10)
elif program == "tblastx":
    from Bio.Blast.Applications import NcbitblastxCommandline
    e_value = 10 ** -20
    blast_cline = NcbitblastxCommandline(query=infas, db=blastdb, evalue=e_value, outfmt=5, out=outfile, num_threads=10)
else:
